...
Record ExponentialMap (M : Manifold3) (g : RiemannianMetric M) := mkExponentialMap {
  exp_map : TangentSpaceType_of M -> sm_type M;
  (* exp_p(0) = p *)
...
Proof.
  ...
  (* Hlb_inf: forall y, In S y -> y >= inf S *)
  (* 应用 Hlb_inf 到 x *)
...
Definition CurveSet (M : Manifold3) (p q : sm_type M) :
...
(* 曲线长度在曲线集合上的像集 *)
Definition CurveLengthImage (M : Manifold3) (g : RiemannianMetric M) (p q : sm_type M) :
  Ensemble R :=
  fun r => exists γ, In (CurveSet M p q) γ /\ r = curve_length M g γ.
...
Definition geodesic_distance_def :
  forall (M : Manifold3) (g : RiemannianMetric M) (p q : sm_type M),
    R :=
...
Lemma dist_tri :
  forall (M : Manifold3) (g : RiemannianMetric M) (p q r : sm_type M),
...
Proof.
  ...
  (* 证明 S₁ 非空 *)
  assert (Hex1: exists y, In (CurveLengthImage M g p q) y).
  { exists (curve_length M g (fun _ => ts_zero_of M)).
    exists (fun _ => ts_zero_of M).
    split.
    ...
    { reflexivity. } }
  (* 证明 S₂ 非空 *)
  assert (Hex2: exists y, In (CurveLengthImage M g q r) y).
  { exists (curve_length M g (fun _ => ts_zero_of M)).
    exists (fun _ => ts_zero_of M).
    split.
    { simpl. exact Coq.Init.Logic.I. }
    { reflexivity. } }
  (* 证明 S₃ 非空 *)
  assert (Hex3: exists y, In (CurveLengthImage M g p r) y).
  { exists (curve_length M g (fun _ => ts_zero_of M)).
    exists (fun _ => ts_zero_of M).
    split.
    { simpl. exact Coq.Init.Logic.I. }
