...
(* 风格: Record + Parameter + Axiom + Lemma, 与 SphereClassification 一致. *)
...
Axiom riemann_skew_symm :
  forall (M : Manifold3) (p : sm_type M)
...
(* ===================================================================== *)
(* 8. Sectional Curvature (截面曲率)                                     *)
(* ===================================================================== *)

(* 截面曲率 K(u,v): 切平面 span(u,v) 上的曲率.
 * 数学公式: K(u,v) = R(u,v,u,v) / (g(u,u)g(v,v) - g(u,v)^2)
 * 分母 = ||u||²||v||² - <u,v>² = ||u×v||² (Gram determinant)
 * 当 u,v 正交单位时: K(u,v) = R(u,v,u,v) *)

Definition sectional_curvature_def :
  forall (M : Manifold3) (g : RiemannianMetric M) (p : sm_type M),
    TangentSpaceType_of M -> TangentSpaceType_of M -> R :=
  fun M g p u v =>
    riemann_04 M g p u v u v
      / (metric_tensor M g p u u * metric_tensor M g p v v
         - metric_tensor M g p u v * metric_tensor M g p u v).

(* Sectional curvature bounded above by C:
 * 命题: 流形 M 在点 p 处所有切截面的曲率 ≤ C. *)
Definition SectionalCurvature_def
    (M : Manifold3) (g : RiemannianMetric M) (p : sm_type M)
    (C : R) : Prop :=
  forall (u v : TangentSpaceType_of M),
    (* u,v 线性无关 (分母 > 0) *)
    (metric_tensor M g p u u * metric_tensor M g p v v
     - metric_tensor M g p u v * metric_tensor M g p u v > 0)%R ->
    (* 该切平面上的截面曲率 ≤ C *)
    (sectional_curvature_def M g p u v <= C)%R.

(* ===================================================================== *)
(* 8b. Compatibility alias for old code                                  *)
(* ===================================================================== *)

(* Old Parameter sectional_curvature replaced by sectional_curvature_def.
 * Alias for backward compatibility with KillingHopf.v and SphereTheorem.v. *)
Definition sectional_curvature_alias
    (M : Manifold3) (g : RiemannianMetric M) (p : sm_type M)
    (u v : TangentSpaceType_of M) : R :=
  sectional_curvature_def M g p u v.

(* ===================================================================== *)
(* 9. Summary                                                            *)
(* ===================================================================== *)

(* Parameters: riemann_tensor, ricci_tensor = 2 *)
(* Definitions: riemann_04, sectional_curvature_def, SectionalCurvature_def,
 *              sectional_curvature_alias = 4 *)
(* Axioms (原始 6):
