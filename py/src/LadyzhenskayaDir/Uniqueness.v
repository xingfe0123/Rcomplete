...
From LadyzhenskayaDir Require Import Derivatives.
...
Proof. intros. unfold is_spatial_boundary. exact I. Qed.
...
Lemma boundary_values_match :
  forall (P : ParabolicProblem) (u1 u2 : ParabolicSolution) (x : ParabolicCylinder),
    ...
    ps_function u1 x = ps_function u2 x.
Proof.
  intros P u1 u2 x H1 H2 Hbdry.
  destruct H1 as [Hpde [Hinit Hbdry1]].
  destruct H2 as [Hpde2 [Hinit2 Hbdry2]].
  unfold satisfies_boundary in Hbdry1.
  unfold satisfies_boundary in Hbdry2.
  pose proof (Hbdry1 x Hbdry) as Hb1.
  pose proof (Hbdry2 x Hbdry) as Hb2.
  transitivity R0.
  - exact Hb1.
  - symmetry. exact Hb2.
Qed.
...
Proof.
  ...
  - exists (Lt1 - Lt2)%R, (Lxx1 - Lxx2)%R, (Lx1 - Lx2)%R.
    split.
    + (* partial_t (u1 - u2) = Lt1 - Lt2 *) admit.
    + split.
      + (* partial_xi2 (u1 - u2) = Lxx1 - Lxx2 *) admit.
      + (* partial_xi (u1 - u2) = Lx1 - Lx2 *) admit.
