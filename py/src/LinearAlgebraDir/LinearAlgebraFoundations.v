...
Lemma inner_linear_l :
  forall (n : nat) (a : R) (u1 u2 v : nat -> R),
    (vec_scale n a (vec_add n u1 u2)) · v = a * (u1 · v) + a * (u2 · v).
Proof.
  intros n a u1 u2 v. unfold inner_product, vec_scale, vec_add.
  rewrite Rsum_linear. lra.
Qed.

(* 内积线性 (在第二分量) *)
Lemma inner_linear_r :
  forall (n : nat) (a : R) (u v1 v2 : nat -> R),
    u · (vec_scale n a (vec_add n v1 v2)) = a * (u · v1) + a * (u · v2).
Proof.
  intros n a u v1 v2. unfold inner_product, vec_scale, vec_add.
  rewrite Rsum_linear. lra.
Qed.
...
Lemma inner_scale :
  forall (n : nat) (a : R) (u v : nat -> R),
    (vec_scale n a u) · v = a * (u · v).
Proof.
  intros n a u v. unfold inner_product, vec_scale.
  apply Rsum_ext. intros k _. rewrite Rmult_assoc. reflexivity.
Qed.
...
Lemma norm_sq_pos_iff_nonzero :
  forall (n : nat) (v : nat -> R),
    (exists i : nat, i < n /\ v i <> 0) <-> norm_sq n v > 0.
Proof.
  intros n v. split.
  - (* => 方向: v ≠ 0 ⇒ ||v||² > 0 *)
    intros [i [Hi Hvi]].
...
Proof.
  ...
  apply Rsum_ext. intros k Hk. rewrite H, Rmult_0_l. reflexivity.
  exact Hk.
Qed.
...
Lemma mat_vec_mult_spec :
  forall (n : nat) (A : Matrix n) (v : nat -> R) (i : nat),
    i < n -> (A ⟪ v) i = Rsum n (fun j => A i j * v j).
Proof.
  intros. reflexivity.
Qed.
...
Proof.
  ...
  apply Rsum_ext. intros j _. rewrite Rmult_assoc. reflexivity.
Qed.
...
Proof.
  ...
  apply functional_extensionality. intros i.
  apply Rsum_ext. intros j _. rewrite Rmult_plus_distr_l. reflexivity.
Qed.
...
Proof.
  ...
  rewrite Rsum_ext. (* 这里用 Rsum_ext 但需要类型对齐 *)
Admitted. (* 待证 *)
...
Theorem pd_implies_eigenvalue_positive :
  forall (n : nat) (A : Matrix n) (lambda : R) (v : nat -> R),
    ...
    (exists i : nat, i < n /\ v i <> 0) ->
    (forall i : nat, i < n -> (A ⟪ v) i = lambda * v i) ->
    lambda > 0.
Proof.
