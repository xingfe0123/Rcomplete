...
(******************************************************************************)
(* (D) 几何级数收敛：|r| < 1 => sum r^n 收敛                                  *)
(* 证明需要几何级数部分和公式 S_n = (1-r^{n+1})/(1-r) 及其极限               *)
(******************************************************************************)
...
(******************************************************************************)
(* (D) 逐项比较：c_k <= d_k 对 n < k <= m 时                                  *)
(* partial_sum c m - partial_sum c n <= partial_sum d m - partial_sum d n    *)
(******************************************************************************)

Axiom partial_sum_diff_le :
  forall (c d : nat -> R) (n m : nat),
    (forall k, (n < k)%nat -> (k <= m)%nat -> c k <= d k) ->
    (forall k, (n < k)%nat -> (k <= m)%nat -> c k >= 0) ->
    (forall k, (n < k)%nat -> (k <= m)%nat -> d k >= 0) ->
    (n <= m)%nat ->
    partial_sum c m - partial_sum c n <= partial_sum d m - partial_sum d n.

(******************************************************************************)
(* 根值收敛法 — 收敛部分                                                       *)
(* 若存在 0 <= r < 1 和 N 使得 n >= N 时 |a_n| <= r^n                        *)
(* 则由比较判别法 + 几何级数收敛，sum a_n 收敛                                *)
(******************************************************************************)

Theorem root_test_convergent :
  forall (a : nat -> R) (r : R),
    0 <= r -> r < 1 ->
...
(******************************************************************************)
(* 根值收敛法 — 发散部分                                                       *)
(* 若存在 r > 1 和 N 使得 n >= N 时 |a_n| >= r^n                             *)
(* 则 a_n 不趋于 0，sum a_n 发散                                              *)
(******************************************************************************)

Theorem root_test_divergent :
  forall (a : nat -> R) (r : R),
    r > 1 ->
    (exists N : nat, forall n : nat, (N <= n)%nat ->
      Rabs (a n) >= r ^ n) ->
    ~ series_cv a.
Proof.
  intros a r Hr [N HN] Hcv.
  assert (Hr_pos : 0 < r) by lra.
  assert (HrN_pos : 0 < r ^ N). { apply pow_lt. exact Hr_pos. }
  (* a_n 不趋于 0 因为 |a_n| >= r^n >= r^N > 0 *)
  assert (Hnot_tendsto_0 : ~ Un_cv (fun n => a n) 0).
  { intro H0. unfold Un_cv, Rdist in H0.
    specialize (H0 (r ^ N) HrN_pos) as [M HM].
    set (k := max N M).
    assert (Hk_N : (N <= k)%nat) by lia.
    assert (Hk_M : (k >= M)%nat) by lia.
    specialize (HN k Hk_N).
    specialize (HM k Hk_M). unfold Rdist in HM.
    assert (Hrk_pos : r ^ k > 0). { apply pow_lt. exact Hr_pos. }
    assert (Hrk_ge_rN : r ^ k >= r ^ N).
    { assert (Hk_ge_N : (k >= N)%nat) by lia.
      revert N Hk_ge_N. induction k as [|k IH].
      - intros N H. simpl in H. lia.
      - intros N H. destruct N as [|N].
        + apply Rle_ge. apply pow_le. lra.
        + simpl. assert (Hk_ge_N : (k >= N)%nat) by lia.
          assert (HIH : r ^ k >= r ^ N) by apply IH.
          assert (Hr_pos2 : r > 0) by lra.
          apply Rmult_le_reg_l with (/ r).
          * apply Rinv_0_lt_compat. exact Hr_pos2.
          * rewrite Rmult_assoc. rewrite Rinv_r.
            -- rewrite Rmult_1_l. exact HIH.
            -- apply Rgt_not_eq. exact Hr_pos2. }
    (* |a_k| >= r^k >= r^N, 但 |a_k - 0| < r^N, 矛盾 *)
    assert (Habs_ak : Rabs (a k) >= r ^ k) by exact HN.
    assert (Hak_lt : Rabs (a k) < r ^ N).
...
(******************************************************************************)
(* 根值收敛法 — limsup 版本                                                   *)
(* 定义 alpha = limsup_{n->infty} |a_n|^{1/n}                                *)
(* alpha < 1 => 收敛, alpha > 1 => 发散                                      *)
(******************************************************************************)

(* (D) limsup 定义和性质 *)
Axiom root_lim_sup : (nat -> R) -> R.

Axiom root_lim_sup_spec :
  forall (a : nat -> R) (alpha : R),
    root_lim_sup a = alpha ->
    (alpha < 1 -> series_cv a) /\
    (alpha > 1 -> ~ series_cv a).

Theorem root_test_limsup :
  forall (a : nat -> R) (alpha : R),
    root_lim_sup a = alpha ->
    (alpha < 1 -> series_cv a) /\
    (alpha > 1 -> ~ series_cv a).
Proof.
  intros a alpha Heq.
  destruct (root_lim_sup_spec a alpha Heq) as [H1 H2].
